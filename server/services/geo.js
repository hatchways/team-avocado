...
const {GEO_APP_ID, GEO_APP_CODE} = process.env;
  ...
  module.exports.getCoordinates = async function (address) {
    ...
    const URL = `https://geocoder.api.here.com/6.2/geocode.json?app_id=${APP_ID}&app_code=${APP_CODE}&searchtext=${address.replace(" ", "+")}`;
    ...
    try {
      result = {
        lat: res.Response.View[0].Result[0].Location.MapView.TopLeft.Latitude,
        lon: res.Response.View[0].Result[0].Location.MapView.TopLeft.Longitude
      };
    } catch (error) {
